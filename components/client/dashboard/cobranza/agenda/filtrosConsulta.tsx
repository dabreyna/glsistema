        ...
        const fetchResumen = async () => {
            try {
                const response = await fetch(
                    `/api/dashboard/cobranza/agenda/listarCitas?fecha=${fecha}&idUsuario=${id_usuario}&perfil=${perfil_usuario}`
                );
                if (!response.ok) {
            ...
            try {
                const response = await fetch(
                    `/api/dashboard/cobranza/agenda/listarCompromisos?fecha=${fecha}&idUsuario=${id_usuario}&perfil=${perfil_usuario}`
                );
                if (!response.ok) {
                    throw new Error(`Failed to fetch data: ${response.status}`);
                }
                const data = await response.json();
                setListadoCompromisos(data);
            } catch (error) {
                console.error(error);
            }
        };
