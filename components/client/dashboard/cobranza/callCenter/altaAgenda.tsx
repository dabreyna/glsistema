...
import TablaMovimientos from "@/components/client/dashboard/cobranza/callCenter/tablaMovimientos";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion";
import { DatosGenerales } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/datosGenerales";
import { Referencias } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/referencias";
import { EstadoCuenta } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/estadoCuenta";
import { Beneficiarios } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/beneficiarios";
import { Atrasos } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/atrasos";
import { ServiciosAtrasados } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/serviciosAtrasados";
import { ServiciosHistorial } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/serviciosHistorial";
import { PagosTerreno } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/pagosTerreno";
import { Amortizacion } from "@/components/client/dashboard/cobranza/callCenter/agendaCompromiso/amortizacion";
import { CompromisosPago } from "./agendaCompromiso/compromisosPago";
...
export function AltaAgenda({ idCliente, listaClasificacion, idContrato, idUsuario, perfilUsuario, clienteNombre }: AltaAgendaProps) {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`/api/dashboard/cobranza/agenda/ultimoComentario?idContrato=${idContrato}`);
            if (!response.ok) {
                throw new Error(`Failed to fetch data: ${response.status}`);
            }
            const data = await response.json();
            // setDataEmpresa(data);
            setUltimoComentario(data[0].comentario);
    ...
    // useEffect(() => {
    //     if (isOpen) {
    //         // Only call fetchData if the dialog is open
    //         fetchData();
    //     }
    // }, [isOpen, idCliente]);
    ...
    useEffect(() => {
        // setListadoCitas([]);
        // setListadoCompromisos([]);
        // datos();
        if (datePayment && filtroMovimientos) {
            calculaMovimientos();
        }
        //TODO:  llamar funcion async donde haga el calculo de la tabla de movimientos respecto a la fecha de pago seleccionada
    ...
    return (
        <>
            <Dialog>
                ...
                <DialogContent className=" md:max-w-full sm:min-w-[430px] lg:max-w-[99%] h-[95%]">
                    <ScrollArea className="h-6/6 w-full rounded-md border p-4">
                        <DialogHeader>
                            <DialogTitle className="text-center uppercase text-black">
                                AGREGAR NUEVO COMPROMISO DE PAGO A: &nbsp;&nbsp;{clienteNombre != "" ? clienteNombre : ""}
                            </DialogTitle>
                            ...
                            <DialogDescription>
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 text-black">
                                    ...
                                    <Separator />
                                    <div className="md:col-span-6 lg:col-span-6 xl:col-span-6 pl-0 bg-slate-200 h-full">
                                        <Label htmlFor="Comentario" className="uppercase text-lg">
                                            último comentario:{" "}
                                        </Label>
                                        {ultimoComentario}
                                    </div>
                                    ...
                                    <div className="md:col-span-6 lg:col-span-6 xl:col-span-6">
                                        ...
                                        {/* TODO: agregar tabla de movimientos de acuerdo a la fecha de pago seleccionada. */}
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 text-black">
                                    <div className="md:col-span-12 lg:col-span-12 xl:col-span-12">
                                        <Accordion type="single" collapsible>
                                            <AccordionItem value="item-1">
                                                <AccordionTrigger className="uppercase hover:bg-secondary">
                                                    Datos Generales
                                                </AccordionTrigger>
                                                <AccordionContent itemScope>
                                                    <DatosGenerales idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-2">
                                                ...
                                                <AccordionContent>
                                                    <CompromisosPago idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-3">
                                                <AccordionTrigger className="uppercase hover:bg-secondary">Referencias</AccordionTrigger>
                                                <AccordionContent>
                                                    <Referencias idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-4">
                                                ...
                                                <AccordionContent>
                                                    <EstadoCuenta idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-5">
                                                <AccordionTrigger className="uppercase hover:bg-secondary">Beneficiarios</AccordionTrigger>
                                                <AccordionContent>
                                                    <Beneficiarios idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-6">
                                                <AccordionTrigger className="uppercase hover:bg-secondary">Atrasos</AccordionTrigger>
                                                <AccordionContent>
                                                    <Atrasos idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-7">
                                                ...
                                                <AccordionContent>
                                                    <ServiciosAtrasados idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-8">
                                                ...
                                                <AccordionContent>
                                                    <ServiciosHistorial idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-9">
                                                ...
                                                <AccordionContent>
                                                    <PagosTerreno idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="item-10">
                                                <AccordionTrigger className="uppercase hover:bg-secondary">Amortizacion</AccordionTrigger>
                                                <AccordionContent>
                                                    <Amortizacion idContrato={idContrato} />
                                                </AccordionContent>
                                            </AccordionItem>
                                        </Accordion>
                                    </div>
