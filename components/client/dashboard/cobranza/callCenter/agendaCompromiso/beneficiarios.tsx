...
export function Beneficiarios({ idContrato }: BeneficiariosProps) {
    const [datosCliente, setDatosCliente] = useState<Beneficiario[]>([]);
    // const [isOpen, setIsOpen] = useState(false);
    const fetchData = async () => {
        try {
            const response = await fetch(`/api/dashboard/cobranza/callCenter/beneficiarios?idContrato=${idContrato}`);
            if (!response.ok) {
                ...
                <Table id="tablaDatos" className="rounded-md border border-slate-200 shadow-sm max-w-[full]">
                    ...
                    <TableHeader className="border border-slate-200 bg-red-700 text-white">
                        <TableRow>
                            ...
                            <TableHead className="text-center p-0 text-white">Tel Oficina</TableHead>
                            ...
                            {datosCliente.map((referencia) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className="text-center uppercase p-1">{referencia.tel_oficina}</TableCell>
