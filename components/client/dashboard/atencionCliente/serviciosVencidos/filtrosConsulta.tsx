...
export default function FiltrosConsultaServiciosVencidos(
    { listaFraccionamientos, listaServicios }: FiltrosConsultaProps,
    { className }: React.HTMLAttributes<HTMLDivElement>
) {
    const seleccionaResultados = useServiciosVencidosFiltrosConsultaStore((state: { setResultados: any }) => state.setResultados);
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(
                    `/api/dashboard/atencionCliente/serviciosVencidos?idFraccionamiento=${fraccionamiento}&fInicio=${fInicio}&fFin=${fFin}&mensualidades=${mensualidades}&nombreCliente=${nombreCliente}&chkDolares=${chkDolares}&chkServicio=true&idServicio=${servicio}`
                );
                if (!response.ok) {
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12">
                ...
                <div className="md:col-span-2 lg:col-span-2 xl:col-span-2">
                    ...
                    <Select onValueChange={setServicio}>
                        <SelectTrigger id="status" aria-label="Selecciona el servicio">
                        ...
                        <SelectContent>
                            ...
                            {listaServicios.map((servicio) => (
                                <SelectItem key={servicio.id_servicio} value={servicio.id_servicio}>
                                    {servicio.servicio}
                                </SelectItem>
                            ))}
                        </SelectContent>
