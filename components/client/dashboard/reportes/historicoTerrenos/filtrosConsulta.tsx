...
export default function FiltrosConsultaHistoricoDeTerrenos({ listaFraccionamientos }: FiltrosConsultaProps) {
    const seleccionaResultados = useHistoricoTerrenosFiltrosConsultaStore((state: { setResultados: any }) => state.setResultados);
    ...
    function getDatos() {
        const fetchData = async () => {
            try {
                const response = await fetch(
                    `/api/dashboard/reportes/historicoTerrenos?idFraccionamiento=${fraccionamiento}&idManzana=${manzana}&idTerreno=${terreno}`
                );
                if (!response.ok) {
                    throw new Error(`Failed to fetch data: ${response.status}`);
                }
